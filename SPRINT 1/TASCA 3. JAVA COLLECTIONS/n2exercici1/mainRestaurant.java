...
public class mainRestaurant {
	...
	public static void main(String[] args) {
		...
		System.out.println(llistatRestaurant.contains(CanPere));
		
		Iterator<Restaurant> itr = llistatRestaurant.iterator();
		while(itr.hasNext()) {
			System.out.println(itr.next());
		}
		
		// restaurant amb el mateix nom pero diferent puntuacio
		llistatRestaurant.add(new Restaurant ("Can Pere",5));
		llistatRestaurant.add(new Restaurant ("Oliva",5));
		
		// restaurant amb el mateix nom i puntuacio
		llistatRestaurant.add(Guinardo);
		llistatRestaurant.add(LaParada);
		
		// printem el contingut del HashSet
